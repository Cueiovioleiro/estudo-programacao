...
| `web01/login.html` | Login e registo (usa `auth-supabase.js`) |
| `web01/auth-supabase.js` | Cliente Supabase e formulários de auth |
| `web01/ui.js` | UI geral (ex.: painel de definições) |
| `web01/script.js` | Lógica da página inicial (ex.: botão “Começar Agora”) |
| `web01/style.css` | Estilos |
| `web01/supabase-config.example.js` | Modelo de configuração (pode ir para o Git) |
| `web01/supabase-config.js` | **Chaves reais — não commitar** (está no `.gitignore`) |
...
2. **Configurar Supabase:** copiar o modelo e preencher com o projeto partilhado no painel Supabase (Project Settings → API):
   - Copiar `web01/supabase-config.example.js` para `web01/supabase-config.js`.
